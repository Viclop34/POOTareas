...
//    metodos
    ...
    public void mostrarConsultasMedico(String idMed){
        int cantidadConsultas = 0;
        for (Consulta consulta : this.listaConsultas) {
            if (consulta.getMedico().getId().equals(idMed)) {
                System.out.println(consulta.mostrarDatosConsulta());
                cantidadConsultas++;
            }
            else if (cantidadConsultas == 0) {
                System.out.println("El medico no tiene consultas");
            }
        }
    }

    public void mostarPacienteDeMedico(String idMed){
        int cantidadPacientes= 0;
        for (Consulta consulta : this.listaConsultas) {
            if (consulta.getMedico().getId().equals(idMed)) {
                System.out.println(consulta.getPaciente().getNombre());
                cantidadPacientes++;
            }
            else if (cantidadPacientes==0) {
                System.out.println("El medico no tiene usuarios.pacientes");
            }
        }
    }
    ...
    public void mostrarConsultasPaciente(String idPaciente){
        ...
        for (Consulta consulta : this.listaConsultas) {
            if (consulta.getPaciente().getId().equals(idPaciente)) {
                ...
                System.out.println("El paciente no tiene consultas");
            }
        }
    }
}
